...
from PostProcess import calculate_metric_simple_inference
...
class Trainer():
    def __init__(self, args, model, train_loader, eval_loader, test_loader, verbose=False):
        ...
        self.modal_used = self.args.modal_used
        print(f"Using modalities: {self.modal_used}")
        ...
        # Test configuration
        if self.args.dataset_name == "Lab_multimodal":
            self.hr_calculate_batch_step = 4
            self.video_Hz = 30
        else:
            raise ValueError("No available test configuration, please check!")
    ...
    def train(self, save_path=None):
        ...
        for epoch in range(self.num_epoch):
            ...
            print(f"Epoch {epoch+1}/{self.num_epoch} | Train/Loss: {loss_epoch:.4f} | Time: {time_cost:.2f}s")
            self.eval_model()
            if (epoch+1) % 5 == 0:
        ...
        if save_path is not None:
            ...
            model_path = model_path + self.args.video_backbone + "_" + self.args.dataset_name + "_model_weights.pth"
    ...
    def test_model(self, epoch):
        ...
        if remaining_samples > 0:
            ...
            # Calculate metrics for remaining samples
            pred_hr_fft, gt_hr_fft = calculate_metric_simple_inference(
        ...
        # Create 6x1 subplots for PPG visualization
        ...
        # Create x-axis ticks, every 12 samples represents 64s
