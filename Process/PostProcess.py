...
def calculate_metric_simple_inference(predictions, labels, original_fps=30):
    ...
    predictions = bandpass_filter(predictions, fs=original_fps) 
    pred_hr = get_hr(predictions, sr=original_fps)

    labels = bandpass_filter(labels, fs=original_fps) 
    gt_hr = get_hr(labels, sr=original_fps)

    return pred_hr, gt_hr
